...
def start_server(host="127.0.0.1", port=8520):
    ...
    if _server is not None:
        print("MILODO chat server already running")
        return _server
    ...
    print(f"MILODO chat server running on http://{host}:{port}")
    return _server
...
def stop_server():
    ...
    if _server is None:
        print("MILODO chat server not running")
        return {
            "success": True,
    ...
    print("MILODO chat server stopped")
    return {
        "success": True,
...
def handle_chat(message):
    ...
    if learn_match:
        ...
        print("[AUTO-SKILL] Création skill")
        result = create_skill(name, description, code)

        if result.get("success"):
            print("[AUTO-SKILL] Succès")
            return {
                "success": True,
                "message": message,
